...
topology topology::load(const std::string &topology_file) {
...
topology topology::parse(const std::string &topology_def) {
  ...
  for (int i = 0; i < d[topo_].Size(); i++) {
    ...
    t.svclasses[d[topo_][i]["service_class"].GetString()] = svc;
  }
  return t;
}
