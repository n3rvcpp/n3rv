...
std::unordered_map<int, std::string> logger::ll_map;
std::unordered_map<std::string, int> logger::fac_map;
...
void logger::add_dest(const char *dest) {
  ...
  if (dst.substr(0, 7) == "file://") {
    ...
    if (syslog_args.size() != 3)
      return;
    // invalid facility
    if (logger::fac_map.find(syslog_args[2]) == logger::fac_map.end())
      return;

    openlog(syslog_args[1].c_str(), LOG_NDELAY,
            logger::fac_map[syslog_args[2]]);
...
void logger::set_loglevel(int lvl) { this->log_level = lvl; }
