...
#if __GNUC__ >= 6
RAPIDJSON_DIAG_OFF(terminate) // ignore throwing RAPIDJSON_ASSERT in
                              // RAPIDJSON_NOEXCEPT functions
#endif
#endif // __GNUC__
...
class GenericMemberIterator
    : public std::iterator<
          ...
          typename internal::MaybeAddConst<
              Const, GenericMember<Encoding, Allocator>>::Type> {
...
public:
  ...
  */
  ...
  /*! Creates a copy of a Value by using the given Allocator
     ...
     Commonly use GenericDocument::GetAllocator(). \see CopyFrom()
  */
  ...
  */
  ...
  /*!
     ...
     with any object is always \c false. \note Linear time complexity (number of
    ...
    if (o.size >= o.capacity) {
      if (o.capacity == 0) {
        ...
        SetMembersPointer(reinterpret_cast<Member *>(
            allocator.Malloc(o.capacity * sizeof(Member))));
      } else {
        SizeType oldCapacity = o.capacity;
        o.capacity += (oldCapacity + 1) / 2; // grow by factor 1.5
        SetMembersPointer(reinterpret_cast<Member *>(
            allocator.Realloc(GetMembersPointer(), oldCapacity * sizeof(Member),
...
public:
  ...
  */
  ...
  GenericDocument &Parse(const typename SourceEncoding::Ch *str,
                         size_t length) {
    RAPIDJSON_ASSERT(!(parseFlags & kParseInsituFlag));
    MemoryStream ms(static_cast<const char *>(str),
...
template <typename Encoding, typename Allocator>
template <typename SourceAllocator>
inline GenericValue<Encoding, Allocator>::GenericValue(
