...
public:
  ...
  std::string get_controller_host() { return this->controller_host; }

  int get_controller_port() { return this->controller_port; }

  std::unordered_map<std::string, n3rv::qconn> get_connections() {
...
int test_service_instanciate() {
  ...
  if (st1.get_controller_host() != "127.0.0.1")
    result = 1;
  if (st1.get_controller_port() != 10001)
    result = 2;
  if (st1.get_connections().size() < 2)
    result = 3;
...
int test_service_connect() {
  ...
  n3rv::service_test st1("127.0.0.1", 10001, ll);
  st1.set_uid("com.class.node1");
  ...
  if (st1.get_connections()[h2->cid].socket == nullptr)
    return 1;
