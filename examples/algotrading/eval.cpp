...
public:
  ...
  int send_order(std::string asset, int size, float stop, float limit) {
    ...
    if (this->connections.find(this->exec->cid) == this->connections.end())
      return 1;
  ...
  static void *process_data(void *objref, zmq::message_t *zmsg) {
    ...
    if (msg.action == "market_price" && msg.args[0] == "DOW_IA") {
      ...
      self->ll->get().log(n3rv::LOGLV_NOTICE, "DOW_PRICE:" + msg.payload);

      if (self->dow_price != 0 && self->dow_price <= 20900) {
        self->ll->get().log(n3rv::LOGLV_NOTICE, "SELL SIGNAL!");
        self->send_order("DOW_IA", -1, -5000, 2000);
      }

      else if (self->dow_price >= 21200) {
        self->ll->get().log(n3rv::LOGLV_NOTICE, "BUY SIGNAL!");
        self->send_order("DOW_IA", 1, -5000, 2000);
      }

      else if (self->dow_price != 0) {
        self->ll->get().log(n3rv::LOGLV_NOTICE, "NO SIGNAL!");
      }
    }
  }
};
...
int main() {
  ...
  e0.ll->get().set_loglevel(n3rv::LOGLV_DEBUG);
  e0.ll->get().add_dest("stdout");
