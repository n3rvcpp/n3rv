...
public:
  ...
  static void *process_mmqueries(void *objref, zmq::message_t *zmsg) {
    ...
    if (msg.action == "market_order") {
      ...
      self->ll->get().log(n3rv::LOGLV_NOTICE, "Processing Market Order..");
  ...
  static void *update_portfolio(void *objref, zmq::message_t *zmsg) {
    ...
    if (msg.action == "market_price") {
      ...
      self->ll->get().log(n3rv::LOGLV_NOTICE, "Recalculating PNL..");
      ...
      for (auto pos : pfolio_copy) {
        ...
        if (pos.asset == msg.args[0]) {
          ...
          if (pos.pnl <= pos.stop) {
            self->ll->get().log(n3rv::LOGLV_NOTICE, "Closing Position (Stop)");
            self->GLOB_PNL += pos.pnl;
            self->portfolio.erase(self->portfolio.begin() + pnum);
          }

          else if (pos.pnl >= pos.limit) {
            self->ll->get().log(n3rv::LOGLV_NOTICE, "Closing Position (Limit)");
            self->GLOB_PNL += pos.pnl;
            self->portfolio.erase(self->portfolio.begin() + pnum);
          }
        }
      ...
      std::stringstream ss;
      ss << "P&L:" << self->GLOB_PNL + self->RUN_PNL
         << " (RUNNING:" << self->RUN_PNL << " )";
      self->ll->get().log(n3rv::LOGLV_NOTICE, ss.str());
    }
  }
};
...
int main() {
  ...
  mm0.ll->get().set_loglevel(n3rv::LOGLV_DEBUG);
  mm0.ll->get().add_dest("stdout");
  mm0.initialize();
  mm0.run();
}