/**
 * This example shows a basic service architecture. 1 ventiler communicating
 * with 2 workers. workers ask for jobs to the ventiler, which give them.
 * workers perform the asked computation (factorial), then display the result on
 * their stdout. Note: for continuous work delivery, just uncomment the line
 * "self->working = false;" in runwork() function."
 */
...
public:
  ...
  void hkloop() {
    ...
    auto bind = n3rv::binding_lookup(this->directory, "com.vent.*.ventiler");

    if (!this->working && std::nullopt != bind) {

      std::cout << "ENTRY EXISTS IN DIR" << std::endl;

      this->ll->get().log(n3rv::LOGLV_DEBUG, "asking for workload..");
  ...
  static void *runwork(void *objref, zmq::message_t *zmsg) {
    ...
    self->ll->get().log(n3rv::LOGLV_DEBUG, "starting work..");
    self->working = true;
    ...
    std::stringstream ss;
    ss << res;
    self->ll->get().log(n3rv::LOGLV_NOTICE, "work result: " + ss.str());
...
public:
  ...
  int initialize() {
    ...
    this->ll->get().log(n3rv::LOGLV_NOTICE, "binding service..");
  ...
  static void *wl_dist(void *objref, zmq::message_t *zmsg) {
    ...
    if (m_.action == "WL_REQ") {
      ...
      self->ll->get().log(n3rv::LOGLV_NOTICE, "distributing new workload..");
...
int main(int argc, char **argv) {
  ...
  if (strcmp(argv[2], "ctl") == 0) {
    ...
    sc.ll->get().add_dest("stdout");
    sc.ll->get().set_loglevel(n3rv::LOGLV_DEBUG);
    sc.ll->get().log(n3rv::LOGLV_NOTICE, "Running Service Controller..");
    sc.run();
    ...
    w1.ll->get().add_dest("stdout");
    w1.ll->get().set_loglevel(n3rv::LOGLV_DEBUG);
    w1.ll->get().log(n3rv::LOGLV_NOTICE, "Running Worker..");
    ...
    v1.ll->get().add_dest("stdout");
    v1.ll->get().set_loglevel(n3rv::LOGLV_DEBUG);
    v1.ll->get().log(n3rv::LOGLV_NOTICE, "Running Ventiler..");
