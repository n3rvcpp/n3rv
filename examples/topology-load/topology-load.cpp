 ...
 * then the loading of a topology file in order to performs bindings/connections
 * automatically. */
...
public:
  std::unordered_map<std::string, n3rv::qhandler *> hdlist;
  ...
  void hkloop() {
    if (this->service_class == "ping") {
      ...
      if (std::nullopt !=
              n3rv::binding_lookup(this->directory, "com.pong.*.pong") &&
          !this->plock) {
        n3rv::message ping_msg;
        ping_msg.action = "PING";
        this->ll->get().log(n3rv::LOGLV_NOTICE, "Sending Ping..");
  ...
  static void *ping_received(void *objref, zmq::message_t *zmsg) {
    ...
    self->ll->get().log(n3rv::LOGLV_NOTICE,
                        "Received ping, sending back\"pong\"");
